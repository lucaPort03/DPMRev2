# AMCISS DPM Firmware — Developer Documentation
...
### `main.c`
Entry point. Initialises GPIO, Ethernet (RMII), and USART3 (115200 Bd, used for `printf` debug output), then hands control to `MX_ThreadX_Init()`. Nothing application-specific lives here.
...
DHCP is scaffolded but not active — a static IP is set directly in `nx_app_thread_entry` and reapplied on reconnect in `App_Link_Thread_Entry`.

#### Packet Structs (`app_netxduo.h`)

Both structs use `#pragma pack(push, 1)` — no padding bytes.
...
All offsets and the velocity/rate constants are defined in `app_netxduo.h`. `DELAY_COUNT` (220) is the ring buffer depth and must be ≥ the largest delay value — currently ~217 packets for coils 17/18 at 390.857 mm offset.
...
These are active issues in the codebase that should be resolved:

**1. DCM2 bypassed in `process_packet`**
The DCM1 branch sets `*dcm_received |= 0x03` instead of `0x01`, which immediately satisfies the "both DCMs received" condition without waiting for DCM2. There is a `//change to 0x01` comment acknowledging this. Similarly, `dcm_received` is initialised to `0x03` rather than `0x00`. Both are temporary workarounds — the intent and any associated risk should be clearly documented, and the fix tracked.

**2. `active_cs` implicit dependency**
`ldc1101_setPowerMode`, `ldc1101_goTo_Lmode`, and `ldc1101_goTo_RPmode` all call `ldc1101_write_noCS` without selecting a CS first, meaning they silently operate on whichever sensor was last active. This is undocumented and potentially a bug if called outside an explicit CS context.

**3. `hal_gpio_csRelease` fragility**
Direct BSRR writes are not regenerated by CubeMX. Any GPIO reassignment will silently break CS deselection for all sensors.

**4. Magic numbers in `ldc1101_init_sensor`**
Register values like `0x90`, `0xA0` are written without using the named constants that already exist in the header, and without comments explaining the configuration choices.

**5. `calculate_LDC_packet_delays` uses 32 individual assignments**
Should be replaced with a loop over a constant array. Currently fragile if coil count changes.

**6. `hal_gpio_intGet` declared but not implemented**
The function is declared in `__ldc1101_hal.h` but its implementation in `__ldc1101_hal.c` is commented out. It is not called anywhere. Either implement or remove.
...
| STM32CubeMX | 6.15.0 |
| Azure RTOS (ThreadX + NetX Duo) | Bundled via STM32 middleware |
